...
function PopupLogin.OnTextSignup()
    ...
    LoginUI:CreatePopupSingup(function (go)
        -- 创建并移动好注册弹出框后，就把隐藏的登录弹出框销毁
        CS.UnityEngine.Object.Destroy(PopupLogin.MonoProxy.gameObject)

        -- 最后给创建出来的PopupSingup游戏对象绑定lua脚本
        go:AddComponent(typeof(CS.MonoProxy)):BindScript("Launch", "UI/Login/PopupSingup")
    end)
end
